...
``azure-functions`` is installed from the package index (PyPI) via the
provider's own dependency pin, so the suite tracks the published SDK and stays
decoupled from any local azure-functions build.
"""
...
def _install_packages(session: nox.Session, editable: bool = False) -> None:
    """Install azure-functions (from PyPI), durabletask, and the provider.
    ...
    stays fast). ``azure-functions`` is resolved from the package index via the
    provider's own dependency pin, so the suite tracks the published SDK rather
    than a local build.
    """
    if editable:
